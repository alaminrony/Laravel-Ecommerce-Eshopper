            ...
            if($request->hasFile('image')){
            	...
            	$fileName = '';
					 ...
					 if($request->hasFile('video')){
						 ...
						 // File::delete('videos/'.$data['current_video']);
					 }
   ...
   public function deleteProductVideo($id){
    ...
    public function products($url=null){
    	...
    	if($categoryDetails->parent_id==0){
    		...
    		$productsAll = Product::whereIn('category_id', $cat_ids)->where('status','1')->orderBy('id','Desc')->paginate(6);
				$breadcrumb  ="<a style='color:#333;' href='/'>Home</a> / <a style='color:#333;' href='".$categoryDetails->url."'>".
    		...
    		$productsAll = Product::where(['category_id'=>$categoryDetails->id])->where('status','1')->orderBy('id','Desc')->paginate(6);
				...
				$breadcrumb  ="<a style='color:#333;' href='/'>Home</a> / <a style='color:#333;' href='".$mainCategory->url."'>".
				$mainCategory->name."</a> / <a style='color:#333;' href='".$categoryDetails->url."'>".
				$categoryDetails->name."</a>";
    	}
    	...
    	return view('products.listing')
			->with(compact('categories','productsAll','categoryDetails','meta_title','meta_description','meta_keywords','breadcrumb'));
    }
    ...
    public function product($id = null){
        ...
        $categories = Category::with('categories')->where(['parent_id' => 0])->get();
				...
				if($categoryDetails->parent_id==0){
					...
					$mainCategory = Category::where('id',$categoryDetails->parent_id)->first();
					$breadcrumb  ="<a style='color:#333;' href='/'>Home</a> / <a style='color:#333;' href='/products/".$mainCategory->url."'>".
					$mainCategory->name."</a> / <a style='color:#333;' href='/products/".$categoryDetails->url."'>".
					$categoryDetails->name."</a> /".$productDetails->product_name;
				}
    ...
    public function getProductPrice(Request $request){
        ...
        $proAttr = ProductsAttribute::where(['product_id'=>$proArr[0],'size'=>$proArr[1]])->first();
				...
				echo $proAttr->price."-".$getCurrencyRates['BD_Rate']."-".$getCurrencyRates['EUR_Rate']."-".$getCurrencyRates['GBP_Rate'];
        echo "#";
        echo $proAttr->stock;
    }
    ...
    public function addtocart(Request $request){
        ...
        // echo "<pre>"; print_r($data); die;
				...
				if($getProductStock->stock<$data['quantity']){
					return redirect()->back()->with('flash_message_error','Required Quantity is not available now');
    ...
    public function cart(){
        ...
        /*echo "<pre>"; print_r($userCart); die;*/
				$meta_title="Shopping Cart | E-shop";
				$meta_description="view shopping cart of e-shopper website";
				$meta_keywords="shopping cart, e-shop";
        ...
        if($request->isMethod('post')){
            ...
            if($shippingCount>0){
                ...
                $shipping->save();
					 ...
					 $pincodeCount = DB::table('pincodes')->where('code',$data['shipping_pincode'])->count();
					 if($pincodeCount==0){
						 return redirect()->back()->with('flash_message_error','Your Country is not available for delivery.');
					 }

            return redirect()->action('ProductsController@orderReview');
        }
        $meta_title="Checkout | E-shop";
				$meta_description="Checkout of your product";
				$meta_keywords="checkout, e-shopper";
    ...
    public function orderReview(){
        ...
        /*echo "<pre>"; print_r($userCart); die;*/
				$meta_title="Order Review | E-shop";
				$meta_description="Order Review of your product";
				$meta_keywords="Order Review, e-shopper";
    ...
    public function placeOrder(Request $request){
        if($request->isMethod('post')){
            ...
            $shippingDetails = DeliveryAddress::where(['user_email' => $user_email])->first();
						...
						$pincodeCount = DB::table('pincodes')->where('code',$shippingDetails->pincode)->count();
						if($pincodeCount==0){
							return redirect()->back()->with('flash_message_error','Your Country is not available for delivery');
						}
...
*/
    ...
    public function thanks(Request $request){
        ...
        DB::table('cart')->where('user_email',$user_email)->delete();
				$meta_title="Thanks | E-shop";
        return view('orders.thanks')->with(compact('meta_title'));
    }
	...
	public function checkPincode(Request $request){
		if($request->isMethod('post')){
			$data = $request->all();
			echo DB::table('pincodes')->where('code','=',$data['pincode'])->count();
			// if($pincode> 0){
			// 	echo "This Country is available for delivery";
			// }else{
			// 		echo "This Country is not available for delivery";
			// }
